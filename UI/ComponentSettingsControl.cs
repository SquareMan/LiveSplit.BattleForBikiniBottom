...
{
    public partial class ComponentSettingsControl : UserControl
    {
        private LiveSplitState _state;
        
        public ComponentSettingsControl(LiveSplitState state)
        {
            InitializeComponent();
        ...
        private void UpdateSplitControls()
        {
            ...
            if (changed)
            {
                ...
                foreach (Split split in AutosplitterSettings.Autosplits)
                {
                    SplitSettingsControl splitBox = new SplitSettingsControl(split);
        ...
        private void RehydratedSettings_Load(object sender, EventArgs e)
        {
            ...
            if(Memory.IsHooked)
            {
                lblRevision.Text = "Dolphin Hooked";
            }
            else
            {
        ...
        {
            SplitSettingsControl draggedControl = (SplitSettingsControl)e.Data.GetData(typeof(SplitSettingsControl));
            SplitSettingsControl coveredControl = flowLayoutSplits.GetChildAtPoint(flowLayoutSplits.PointToClient(new Point(e.X, e.Y))) as SplitSettingsControl;
            ...
            if (draggedControl != coveredControl && coveredControl != null) {
                ...
                if (Math.Abs(draggedIndex - coveredIndex) > 1)
                {
                    ...
                    coveredControl = (SplitSettingsControl)flowLayoutSplits.Controls[coveredIndex];
                }
